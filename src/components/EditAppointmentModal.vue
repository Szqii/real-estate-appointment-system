        ...
        <form class="p-4 md:p-5" @submit.prevent="updateAppointment">
          <div class="grid gap-4 mb-4">
            ...
            <div class="col-span-2">
              ...
              <AgentSelect 
                :initialSelected="agentIds" 
                @update:selectedAgents="(ids) => agentIds = ids"/>
            </div>

            <!-- Date & Time -->
            <div class="col-span-2">
              <label class="block mb-2 text-sm font-medium text-gray-900">Appointment Date & Time</label>
              <DateSelect 
                :initialSelected="appointmentDate" 
            ...
            <div v-if="relatedAppointments.length > 0" class="col-span-2">
              <label class="block mb-2 text-sm font-medium text-gray-900">Related Appointments</label>
              <div class="max-h-32 overflow-y-auto border border-gray-200 rounded-lg p-3">
                <div v-for="relatedApp in relatedAppointments" :key="relatedApp.id" 
...
const relatedAppointments = computed(() => {
  ...
  return store.appointments.filter(app => 
...
// Watch for appointment prop changes to populate form
watch(() => props.appointment, (newAppointment) => {
  if (newAppointment && props.isModalOpen) {
    appointmentDate.value = newAppointment.appointmentDate;
    appointmentAddress.value = newAppointment.appointmentAddress || "";
    contactIds.value = newAppointment.contacts?.map(c => c.id) || [];
    agentIds.value = newAppointment.agents?.map(a => a.id) || [];
    appointmentStatus.value = newAppointment.isCancelled ? 'Cancelled' : newAppointment.status;
    errors.value = {};
  }
}, { immediate: true });
